...
//    r._id in the Reports tab). 'verify' credits the user's chosen CT pack
  ...
  try {
    ...
    if (action === 'list') {
      ...
      const reports = docs.map(p => ({
        ...
        chosenPlan:  (resolvePack(p.chosenPlan) || {}).id || p.chosenPlan,
        packLabel:   (resolvePack(p.chosenPlan) || {}).label || String(p.chosenPlan || ''),
    ...
    if (action === 'respond') {
      ...
      if (decision === 'verify') {
        ...
        // Excess (if any) becomes an outstanding pending_revenue balance, tied
        // to the notification id, so Tip/Refund on it can find it. (This was
        // missing here before — same bug receive-sms.js once had.)
        const notifId = crypto.randomUUID();
        if (outcome.excess > 0) {
          await db.collection('pending_revenue').insertOne({
        ...
        return { statusCode: 200, body: JSON.stringify({ success: true, userId: pending.userId, amount: pending.claimedAmount, plan: pack.id, tokens: pack.tokens, packLabel: pack.label, excess: outcome.excess }) };
      }
